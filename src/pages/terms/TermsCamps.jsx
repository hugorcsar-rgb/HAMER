// src/pages/terms/TermsCamps.jsx
// Terms & Conditions page for HAMER Camps (Audit, Build, Retainer).
import { Link } from 'react-router-dom';
import {
  campsPackagesLegal,
  CLAUSE_PARTIES_BUSINESS,
  CLAUSE_PRIVACY,
...
function PackageBlock({ pkg }) {
  return (
    <article id={pkg.id} className="mb-24 pb-20 border-b border-foreground/10 last:border-b-0">
      ...
      {pkg.deliveryTimeline && <Clause {...pkg.deliveryTimeline} />}
...
export default function TermsCamps() {
  return (
    <section className="px-[24px] md:px-[89px] pt-32 md:pt-40 pb-24 max-w-[1100px] mx-auto">
      <Link to="/camps" className="font-data text-[11px] tracking-[0.2em] uppercase text-foreground/60 hover:text-accent">
        ← Back to Camps
      </Link>
      ...
      {campsPackagesLegal.map((pkg) => <PackageBlock key={pkg.id} pkg={pkg} />)}
      ...
      <div id="common-terms" className="pt-12 mt-12 border-t border-foreground/15">
        ...
        <Clause {...CLAUSE_PARTIES_BUSINESS} />
